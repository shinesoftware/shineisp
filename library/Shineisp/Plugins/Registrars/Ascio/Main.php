...
class Shineisp_Plugins_Registrars_Ascio_Main extends Shineisp_Plugins_Registrars_Base implements Shineisp_Plugins_Registrars_Interface {
		...
		if(!empty($domain[0])){
			...
			if(!empty($nicHandle)){
				...
				if ($domain[0]['DomainsTlds']['WhoisServers']['tld'] == "it") {
					...
					$params[] = $registrar ['ascio_testmode'] ? true : false; // enable the TEST MODE when enabled (true), will not debit your account
	...
	public Function renewDomain($domainID) {
		...
		if(!empty($domain[0])){
			$domain_name = $domain[0]['domain'] . "." . $domain[0]['DomainsTlds']['WhoisServers']['tld'];
			$soap->resellerDomainRenew ( $this->session['id'], $domain_name, $registrar ['ascio_testmode'] );
			return true;
		}
