...
class Invoices extends BaseInvoices
{
    ...
    public static function getSummaryGrid($helper, $year)
    {
        ...
        $config ['datagrid'] ['columns'] [] = array('label' => $translator->translate('Month Number'), 'field' => 'MONTH(o.order_date)', 'alias' => 'month', 'sortable' => false, 'searchable' => false, 'type' => 'string');
...
// 			Invoice already exists, we return it
        ...
        $database ['columns'] [] = array("value" => $translator->translate("SKU"), "size" => 10, "align" => "left", "key" => "sku");
        ...
        $database ['columns'] [] = array("value" => $translator->translate("Tax"), "size" => 9, "align" => "right", "key" => "taxes");
        $database ['columns'] [] = array("value" => $translator->translate("Discount"), "size" => 10, "align" => "right", "key" => "discount");
        $database ['columns'] [] = array("value" => $translator->translate("Setup fee"), "size" => 10, "align" => "right", "key" => "setup");
        $database ['columns'] [] = array("value" => $translator->translate("Total"), "size" => 15, "align" => "right", "key" => "total");
        ...
        if (isset ($order [0])) {
            ...
            foreach ($order [0] ['OrdersItems'] as $item) {
                ...
                if (!empty($billingCycle)) {
                    if (!empty($billingCycle['months']) && $item['Products']['type'] != "domain") {
                        $price = (($item['price'] + $item['vat']) * $billingCycle['months']) + $item['setupfee'];
                    } else {
                        $price = $item['price'] + $item['vat'] + $item['setupfee'];
                    }
                } else {
                    $price = (($item['price'] + $item['vat']) * $item['quantity']) + $item['setupfee'];
                }
                ...
                $rowtotal = $currency->toCurrency($price, array('currency' => Settings::findbyParam('currency')));
