...
class Tickets extends BaseTickets {
    ...
    public static function grid($rowNum = 10) {
        ...
        $config ['datagrid'] ['fields'] = "t.ticket_id,
											...
											s.status as status, 
											DATE_FORMAT(t.date_open, '".Settings::getMySQLDateFormat('ticketdateformat')." %H:%i') as creation_date, 
											DATE_FORMAT(t.date_updated, '".Settings::getMySQLDateFormat('ticketdateformat')." %H:%i') as updated_at, 
        ...
        $config ['datagrid'] ['massactions']['common'] = array (
            'bulkxmlexport' => 'XML Export',
            'bulkpdfexport' => 'PDF Export',
            'massdelete' => 'Delete' );
    ...
    public static function Last($customerid = "", $limit=10) {
        ...
        $dq = Doctrine_Query::create ()
            ->select ( "t.ticket_id, 
											t.subject as subject, 
											tc.category as category, 
											DATE_FORMAT(t.date_updated, '".Settings::getMySQLDateFormat('ticketdateformat')." %H:%i') as updated,
    ...
    public static function getList($empty = false) {
        ...
        $records = Doctrine_Query::create ()
            ->select ( "ticket_id, DATE_FORMAT(date_open, '".Settings::getMySQLDateFormat('ticketdateformat')."') as date_open, subject" )
            ->from ( 'Tickets t' )
            ->leftJoin ( 't.Customers c' )
            ->addWhere( "c.isp_id = ?", Isp::getCurrentId() )
            ->execute ( array (), Doctrine_Core::HYDRATE_ARRAY );
    ...
    public static function getListbyCustomerId($customer_id, $empty = false, $abbreviation=false) {
        ...
        $records = Doctrine_Query::create ()->select ( "ticket_id, DATE_FORMAT(date_open, '".Settings::getMySQLDateFormat('ticketdateformat')."') as date_open, subject, s.status as status" )
            ->from ( 'Tickets t' )
            ->leftJoin ( 't.Statuses s' )
            ->leftJoin ( 't.Customers c' )
     ...
     * @param array $items
     */
    public function bulkpdfexport($items) {
        $isp = Isp::getActiveISP();
    ...
    public function bulkxmlexport($items=array()) {
        ...
        // Get the records from the tickets table
        $data = self::get_tickets($items);

        $xml = new ExSimpleXMLElement('<shineisp></shineisp>');
        $tickets = $xml->addChild('tickets');

        foreach ($data as $item){

            $ticket = $tickets->addChild('ticket');
            $ticket->addAttribute('id', $item['ticket_id']);
            ...
            if(!empty($item['TicketsCategories']['category'])){
                $category = $item['TicketsCategories']['category'];
            }else{
                $category = "";
            }
            $category = $ticket->addChild('category', $category);
            ...
            $ticket->addChild('date_updated',$item['date_updated']);

            $ticketNotes = self::Notes($item['ticket_id'], null, true);
        ...
        $xml->saveXML(PUBLIC_PATH . "/tmp/tickets.xml");

        if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            die(json_encode(array('url' => "/tmp/tickets.xml")));
        }
