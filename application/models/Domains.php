...
class Domains extends BaseDomains {
    ...
    public static function grid($rowNum = 10) {
        ...
        $massactions['bulkxmlexport'] = 'XML Export';
...
// 	}
    ...
    public function bulkxmlexport($items=array()) {
        ...
        $data = self::get_domains($items);

        $xml = new ExSimpleXMLElement('<shineisp></shineisp>');
        $domains = $xml->addChild('domains');
        ...
        foreach ($data as $item){
            ...
            if(!empty($c)){
                $customer->addAttribute('customer_id', $c['customer_id']);
                $customer->addAttribute('vat', $c['vat']);
                $customer->addAttribute('taxpayernumber', $c['taxpayernumber']);
                $customer->addChildCData('company', $c['company']);
                $customer->addChildCData('fullname', $c['full_personname']);
        ...
        if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            die(json_encode(array('url' => "/tmp/domains.xml")));
        }
