...
class Cart implements Iterator, Countable {
	...
	public function addDomain(CartItem $item, $domain, $tld_id, $mode="register") {
		...
		if (!empty($domain)) {
			$this->items[$id]['domain']['name'] = $domain;
			$this->items[$id]['domain']['tld'] = $tld_id;
			$this->items[$id]['domain']['mode'] = $mode;
		}
	...
	public function checkIfHostingProductIsPresentWithinCart() {
		if (! empty ($this->items ) && is_array ( $this->items)) {
			foreach ( $this->items as $item ) {
				$type = Products::getProductType($item['item']->getId());
				if ($type == "hosting") {
					return true;
				}
			}
	...
	public static function addItem2($productId, $qtyId=1) {
		...
		if (empty ( $productId ) || !is_numeric ( $productId )) { return false; }
