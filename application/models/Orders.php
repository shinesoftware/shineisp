...
class Orders extends BaseOrders {
	 ...
	 * @return ArrayObject
	 */
	public static function incomeGraphMonthly($year = null ) {
		$baseArray  = array('grandtotal' => 0, 'net_total' => 0, 'vat' => 0, 'growPercent' => 0, 'growDiff' => 0);
		$yearIncome = array();
		
		$currentYear = !empty($year) ? $year : date("Y");
		$lastYear    = $currentYear -1;

		// Get the invoices montly total and subtract the credit memo
		$incomes = Doctrine_Query::create ()->select ( "invoice_id, MONTH(i.invoice_date) as month, YEAR(i.invoice_date) as year, SUM(o.grandtotal) as gross_grandtotal, SUM(o.total) as gross_total, SUM(o.vat) as gross_vat,
																	SUM(cm.total) as creditmemo_grandtotal, SUM(cm.total_net) as creditmemo_total, SUM(cm.total_vat) as creditmemo_vat,
																   (SUM(o.grandtotal) - ifnull(SUM(cm.total), 0)) as net_grandtotal,
																   (SUM(o.total) - ifnull(SUM(cm.total_net), 0)) as net_total,
																   (SUM(o.vat) - ifnull(SUM(cm.total_vat), 0)) as net_vat" )
		...
		foreach ( $incomes as $income ) {
			...
			// Subtract the Outcome (Purchase Invoices) of the year
			$costs = PurchaseInvoices::getSummary($income['year'], false, false, true);
			if(!empty($costs[0])){
			
				$income['net_total'] -= $costs[0]['total'];
				$income['net_vat'] -= $costs[0]['vat'];
				$income['net_grandtotal'] -= $costs[0]['grandtotal'];
			...
			if ( $year == $currentYear && $income['net_total'] > 0) {
				$diff    = $income['net_total'] - $yearIncome[$lastYear][$month]['net_total'];
				$percent = round($diff / $income['net_total'] * 100);		
