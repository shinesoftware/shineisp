...
// 			->andWhere('TIMEDIFF(resetpwd_expire, NOW()) > 0')
    ...
    public function bulk_magento($items=array()) {
        ...
        foreach ($data as $item){
            ...
            // Create empty default customer values only if the customer has not any contact information
            $customers[$i]['_address_city'] = '-';
            $customers[$i]['_address_company'] = '';
            $customers[$i]['_address_country_id'] = 'IT';
            $customers[$i]['_address_firstname'] = $item['firstname'];
            $customers[$i]['_address_lastname'] = $item['lastname'];
            $customers[$i]['_address_middlename'] = '';
            $customers[$i]['_address_postcode'] = '';
            $customers[$i]['_address_prefix'] = '';
            $customers[$i]['_address_region'] = '';
            $customers[$i]['_address_street'] = '-';
            $customers[$i]['_address_suffix'] = '';
            $customers[$i]['_address_telephone'] = '0000000';
            $customers[$i]['_address_fax'] = '0000000';
            $customers[$i]['_address_vat_id'] = $item['vat'];
            ...
            foreach ($item['Addresses'] as $addr){
                ...
                $customers[$i]['_address_prefix'] = '';
                $customers[$i]['_address_region'] = '';
                $customers[$i]['_address_street'] = !empty($addr['address']) ? $addr['address'] : "-";
                $customers[$i]['_address_suffix'] = '';

                foreach ($item['Contacts'] as $cnt){
                    if($cnt['ContactsTypes']['name'] == "Telephone"){
                        $customers[$i]['_address_telephone'] = $cnt['contact'];
                    }elseif ($cnt['ContactsTypes']['name'] == "Fax"){
                        $customers[$i]['_address_fax'] = $cnt['contact'];
                    }
                }
