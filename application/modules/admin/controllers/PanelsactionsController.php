...
/**
 * PanelsActions
 * Manage the panelsactions items table
 * @version 1.0
 */

class Admin_PanelsActionsController extends Shineisp_Controller_Admin {
	
	protected $panelsactions;
	protected $datagrid;
	protected $session;
	...
	public function preDispatch() {
		...
		$this->panelsactions = new PanelsActions();
		$this->translator = Shineisp_Registry::getInstance ()->Zend_Translate;
		$this->datagrid = $this->_helper->ajaxgrid;
		$this->datagrid->setModule ( "panelsactions" )->setModel ( $this->panelsactions );				
	}
	...
	public function indexAction() {
		...
		$redirector->gotoUrl ( '/admin/panelsactions/list' );
	}
	...
	public function listAction() {
		...
		$this->view->buttons = array(array("url" => "/admin/panelsactions/new/", "label" => $this->translator->translate('New'), "params" => array('css' => array('btn'))));
	...
	public function loadrecordsAction() {
		$this->_helper->ajaxgrid->setConfig ( PanelsActions::grid() )->loadRecords ($this->getRequest ()->getParams());
	}
	...
	public function searchprocessAction() {
		$this->_helper->ajaxgrid->setConfig ( PanelsActions::grid() )->search ();
	}
	...
	public function newAction() {
		...
		$this->view->buttons = array(array("url" => "#", "label" => $this->translator->translate('Save'), "params" => array('css' => array('btn'), 'id' => 'submit')),
								array("url" => "/admin/panelsactions/list", "label" => $this->translator->translate('List'), "params" => array('css' => array('btn'))));
	...
	public function deleteAction() {
		...
		if (is_numeric ( $id )) {
			PanelsActions::deleteItem( $id );
		}
		return $this->_helper->redirector ( 'index', 'panelsactions' );
	}
	...
	public function editAction() {
		...
		$this->view->buttons = array(
				...
				array("url" => "/admin/panelsactions/list", "label" => $this->translator->translate('List'), "params" => array('css' => array('btn'), 'id' => 'submit')),
				array("url" => "/admin/panelsactions/new/", "label" => $this->translator->translate('New'), "params" => array('css' => array('btn'))),
		);
		...
		if (! empty ( $id ) && is_numeric ( $id )) {
			...
			if (! empty ( $rs[0] )) {
				...
				$this->view->buttons[] = array("url" => "/admin/panelsactions/confirm/id/$id", "label" => $this->translator->translate('Delete'), "params" => array('css' => array('btn')));
			}
		}
	...
	public function processAction() {
		...
		if (! $request->isPost ()) {
			return $this->_helper->redirector ( 'list', 'panelsactions', 'admin' );
		}
		...
		if ($form->isValid ( $request->getPost () )) {
			...
			// Get the id 
			$id = PanelsActions::saveData($params, $params ['action_id']);
			if (is_numeric ( $id )) {
				$this->_helper->redirector ( 'edit', 'panelsactions', 'admin', array ('id' => $id, 'mex' => $this->translator->translate ( "The task requested has been executed successfully." ), 'status' => 'success' ) );
			} else {
				$redirector->gotoUrl ( "/admin/panelsactions/list/" );
			}
		} else {
	...
	private function getForm($action) {
		$form = new Admin_Form_PanelsActionsForm ( array ('action' => $action, 'method' => 'post' ) );
		return $form;
	}
