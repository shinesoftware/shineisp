...
/**
 * WikiCategoriesController
 * Manage the Wiki category table
 * @version 1.0
 */
...
class Admin_WikicategoriesController extends Shineisp_Controller_Admin {
	...
	public function preDispatch() {
		...
		$this->wikicategories = new WikiCategories ();
		$this->translator = Shineisp_Registry::getInstance ()->Zend_Translate;
		$this->datagrid = $this->_helper->ajaxgrid;
		$this->datagrid->setModule ( "wikicategories" )->setModel ( $this->wikicategories );		
	}
	...
	public function indexAction() {
		...
		$redirector->gotoUrl ( '/admin/wikicategories/list' );
	}
	...
	public function listAction() {
		...
		$this->view->buttons = array(array("url" => "/admin/wikicategories/new/", "label" => $this->translator->translate('New'), "params" => array('css' => array('btn'))));
	...
	public function loadrecordsAction() {
		$this->_helper->ajaxgrid->setConfig ( WikiCategories::grid() )->loadRecords ($this->getRequest ()->getParams());
	}
	...
	public function newAction() {
		...
		$this->view->buttons = array(array("url" => "#", "label" => $this->translator->translate('Save'), "params" => array('css' => array('btn'), 'id' => 'submit')),
									 array("url" => "/admin/wikicategories/list", "label" => $this->translator->translate('List'), "params" => array('css' => array('btn'))));
	...
	public function resetAction() {
		...
		unset ( $NS->search_wikicategories );
		$this->_helper->redirector ( 'index', 'wikicategories' );
	}
	...
	public function confirmAction() {
		...
		try {
			if (is_numeric ( $id )) {
				...
				$this->view->description = $this->translator->translate ( 'If you delete the wiki category, the record will no longer be available.' );
				
				$record = $this->wikicategories->find ( $id );
	...
	public function deleteAction() {
		$id = $this->getRequest ()->getParam ( 'id' );
		try {
			$this->wikicategories->find ( $id )->delete ();
		} catch ( Exception $e ) {
			$this->_helper->redirector ( 'list', 'wikicategories', 'admin', array ('mex' => $this->translator->translate ( 'Unable to process the request at this time.' ) . ": " . $e->getMessage (), 'status' => 'error' ) );
		}
		return $this->_helper->redirector ( 'list', 'wikicategories', 'admin' );
	}
	...
	public function editAction() {
		$form = $this->getForm ( '/admin/wikicategories/process' );
		$id = $this->getRequest ()->getParam ( 'id' );
		
		// Create the buttons in the edit form
		$this->view->buttons = array(
				array("url" => "#", "label" => $this->translator->translate('Save'), "params" => array('css' => array('btn'), 'id' => 'submit')),
				array("url" => "/admin/wikicategories/list", "label" => $this->translator->translate('List'), "params" => array('css' => array('btn'), 'id' => 'submit')),
				array("url" => "/admin/wikicategories/new/", "label" => $this->translator->translate('New'), "params" => array('css' => array('btn'))),
		);
		
		if (! empty ( $id ) && is_numeric ( $id )) {
			$rs = $this->wikicategories->getAllInfo ( $id, null, true );
			
			if (! empty ( $rs [0] )) {
				$form->populate ( $rs [0] );
			}
			
			$this->view->buttons[] = array("url" => "/admin/wikicategories/confirm/id/$id", "label" => $this->translator->translate('Delete'), "params" => array('css' => array('btn')));
	...
	public function processAction() {
		...
		if ($form->isValid ( $request->getPost () )) {
			...
			// Set the new values
			if (is_numeric ( $id )) {
				$this->wikicategories = Doctrine::getTable ( 'WikiCategories' )->find ( $id );
			}
			...
			try {
				...
				$this->wikicategories->category = $params ['category'];
				$this->wikicategories->public = $params ['public'];

				$this->wikicategories->save ();
				$id = is_numeric ( $id ) ? $id : $this->wikicategories->getIncremented ();
				
				$this->_helper->redirector ( 'edit', 'wikicategories', 'admin', array ('id' => $id, 'mex' => 'The task requested has been executed successfully.', 'status' => 'success' ) );
			
			} catch ( Exception $e ) {
				$this->_helper->redirector ( 'edit', 'wikicategories', 'admin', array ('id' => $id, 'mex' => $this->translator->translate ( 'Unable to process the request at this time.' ) . ": " . $e->getMessage (), 'status' => 'error' ) );
			}
	...
	private function getForm($action) {
		$form = new Admin_Form_WikiCategoriesForm ( array ('action' => $action, 'method' => 'post' ) );
		return $form;
	}
		...
		if (is_array ( $items ) && is_numeric ( $status )) {
			foreach ( $items as $categoryid ) {
				if (is_numeric ( $categoryid )) {
					$this->wikicategories->set_status ( $categoryid, $status );
				}
			}
			return true;
		}
